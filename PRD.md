...
- **日程表（Schedule）**：把任务按场景/计划分组（如「工作日计划」「周末计划」）。
    ...
    - 删除日程表时，其下任务解除归属（变为未分组），不会被删除。
    - 首次启动若无任何日程表，自动创建一个「默认日程表」并设为运行中。

### 2.2 任务管理（每日任务模式）
- **左右分栏布局**：
    - 左栏：当前日程表下的任务列表（名称、类型、状态、启用开关），随运行时间自动排序，支持拖拽调整顺序。
    - 右栏：选中任务的详情面板，可编辑保存（名称、命令、工作目录、类型、触发配置、超时、启用、备注），可立即执行并查看输出，可删除。
- **新建任务**：通过弹窗完成，不影响主页面布局；表单按后端返回的 schema 动态渲染。
- **任务属性**：名称、命令/脚本路径、任务类型（当前仅支持"每日定时"）、触发配置（时、分）、工作目录（可选）、超时（可选，默认 60s）、备注（可选）、启用/禁用。
- **立即执行**：手动触发一次任务，不受定时限制。

### 2.3 任务类型扩展机制
- **设计原则**：新增任务类型只需实现一个接口（`TaskTypeHandler`），用 `@Component` 注册为 Spring Bean，无需修改已有代码。
- **当前实现**：`DailyTaskTypeHandler`（每日指定时间执行）。
- **预留类型**（未来可加）：一次性、固定间隔、Cron、依赖触发等。
...
- `DELETE /api/schedules/{id}` — 删除日程表（其下任务解除归属）
...
## 7. 界面简要描述
- **模式切换滑块**：顶部 `每日任务` / `立即任务`。
- **每日任务页**：左侧日程表选择条（双击切换运行、右键删除）+ 左右分栏。
    - 左栏任务列表：名称、类型徽标、状态点、启用迷你开关，可拖拽排序。
    - 右栏详情：编辑表单 + 立即执行 + 最近执行输出 + 删除。
- **新建/编辑弹窗**：动态表单（名称、命令、类型下拉、按 schema 渲染的触发配置、备注）。保存后刷新列表。
- **立即任务页**：占位卡片（建设中）。
